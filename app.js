document.addEventListener("DOMContentLoaded", () => {
   ...
   * 弱・強示唆
   *
   * 回数が増えるほど設定予測への影響を強くする。
   * ただし、これは「示唆が出た回数＝その設定の確率」
   * という単純計算ではない。
   *
   * あくまで設定候補を補正するための係数。
   */
  ...
  const softHints = {
    ...
    char_medusa:  { odd: 0.050 },

    char_pekora:  { even: 0.025 },
    char_poporon: { even: 0.050 },

    char_yurine_a: { high: 0.030 },
    char_yurine_b: { high: 0.060 },
  ...
  document.querySelectorAll(".counter-btn").forEach(button => {
    ...
    button.addEventListener("click", () => {
      ...
      if (Object.prototype.hasOwnProperty.call(state, target)) {
        ...
        state[target] =
          Math.max(0, state[target] + amount);
  ...
  function baseProbabilities(games, bonusCount) {
    ...
    const logs =
      bonusRates.map(rate => {
        ...
        return (
          bonusCount * Math.log(lambda) -
          lambda -
          logFactorial(bonusCount)
        );
      });
  ...
  function updatePrediction(probabilities) {
    ...
    if (candidates.length === 1) {
      ...
      element.textContent =
        `設定予測：設定${candidates[0]}が最有力`;

    } else {

      element.textContent =
        `設定予測：設定${candidates[0]}～${candidates[candidates.length - 1]}が候補`;
    }
  }
